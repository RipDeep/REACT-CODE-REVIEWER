...
const CodeAnalysisPanel = ({ data, updateParentVal, setLoadingParent }) => {
  ...
  function loadSettingsVal(theme, fontSize, tabSize, autoSave, lineWrap) {
    console.log("bap hu ");
  ...
  //   if (t.includes("light")) return "vs-light";
  //   if (t.includes("monokai")) return "vs-dark"; // Monaco doesn't have monokai by default
  //   if (t.includes("solarized")) return "vs-dark"; // Monaco doesn't have solarized by default
  //   return "vs-dark";
  // }
  // useEffect(() => {
  ...
  function saveCodeHistory(newEntry) {
    const maxHistory = 6;

    // Get existing history from localStorage
    let history = JSON.parse(localStorage.getItem("userCodeData")) || [];

    // Make sure history is an array
    if (!Array.isArray(history)) history = [];

    // Add new entry at the start (unshift)
    history.unshift(newEntry);

    // Remove duplicates if you want unique code entries
    history = history.filter((entry, index, self) =>
      index === self.findIndex((e) => JSON.stringify(e) === JSON.stringify(entry))
    );
    ...
    localStorage.setItem("userCodeData", JSON.stringify(history));
  }
  ...
  return (
    ...
    <div className="bg-gray-800/50 backdrop-blur-md rounded-2xl border border-gray-700 shadow-lg flex flex-col min-h-[700px] overflow-hidden relative">
      ...
      {/* calling navbar from here to fix z-index issue */}
      <div className="hidden">
        <Navbar
          ReanalyzeParent={Reanalyze}
          loadSettingsValParent={loadSettingsVal}
        />
